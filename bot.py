...
def send_crypto_signal(coin_name, direction, strategy, entry, leverage, tp1, tp2, tp3, tp4, sl):
...
def analyze_and_trade():
    print("Starting scan with Dynamic Leverage & 3 Strategies...")
    exchange = ccxt.mexc()
    ...
    for symbol in SYMBOLS:
        try:
            ...
            if ema_buy or macd_buy or bb_buy:
                ...
                send_crypto_signal(symbol, "LONG", strategy_name, entry, lev, round(entry * 1.0075, decimals), round(entry * 1.017, decimals), round(entry * 1.032, decimals), round(entry * 1.058, decimals), round(entry * 0.95, decimals))
                time.sleep(2)
            ...
            # ✨ SHORT Signals
                ...
                entry = round(current_close, decimals)
                send_crypto_signal(symbol, "SHORT", strategy_name, entry, lev, round(entry * 0.9925, decimals), round(entry * 0.983, decimals), round(entry * 0.968, decimals), round(entry * 0.942, decimals), round(entry * 1.05, decimals))
                time.sleep(2)
            else:
